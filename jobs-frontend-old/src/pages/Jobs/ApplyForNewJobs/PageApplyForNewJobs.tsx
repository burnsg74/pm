...
export default () => {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await fetch(`${API_BASE_URL}/api/job/get-job-saved`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const AppliedJob = async () => {
        ...
        try {
            const updatedJob: Job = {...currentJob, status: "Applied"};
    ...
    return (
        <div className={`${styles.container}`}>
            <Breadcrumb>
                ...
                <Breadcrumb.Item active>Apply for Jobs</Breadcrumb.Item>
            </Breadcrumb>
            {currentJob && (
                <>
                    <h4>
                    ...
                    <Table striped borderless className={styles.tableAutoFit}>
                        ...
                        {currentJob.salary_min && currentJob.salary_min && (
                            <tr>
                                <th>Salary Min :</th>
                                <td>{formatMoney(currentJob.salary_min)}</td>
                                <th>Salary Max :</th>
                                <td>{formatMoney(currentJob.salary_max ?? null)}</td>
                                <td></td>
                            </tr>
                        )}
                        <tr>
